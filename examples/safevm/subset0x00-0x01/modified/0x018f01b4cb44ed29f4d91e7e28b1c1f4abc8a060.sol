...
library SafeMath {
    function mul(uint a, uint b) pure internal returns(uint) {
        uint c = a * b;
        assert(a == 0 || c / a == b); // SAFEVM Integer Semantics
        return c;
    }
    ...
    function add(uint a, uint b) pure internal returns(uint) {
        uint c = a + b;
        assert(c >= a && c >= b); // SAFEVM SAFEMATH
        return c;
    }
}
...
contract CrowdSale is  Pausable, Vesting {
    ...
    enum Step {  // SAFEVM enum data type producing INVALID instructions
    ...
    function numberOfBackers() public view returns(uint, uint, uint, uint) {
        ...
        for (uint i = 0; i < backersIndex.length; i++) {
            Backer storage backer = backers[backersIndex[i]]; // SAFEVM SAFE ACCESS
    ...
    function refund() external {
        ...
        assert(totalEtherReceived > 0); // SAFEVM Could be require?
...
contract Token is ERC20,  Ownable {
    ...
    function mint(address _target, uint256 _mintedAmount) public onlyAuthorized() returns(bool) {
        assert(totalSupply.add(_mintedAmount) <= 1975e23);  // ensure that max amount ever minted should not exceed 197.5 million tokens with 18 decimals // SAFEVM check overflow
