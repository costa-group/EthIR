...
// File: contracts/loopring/helper/MiningHelper.sol
...
// File: contracts/loopring/helper/ParticipationHelper.sol
...
library ParticipationHelper {
        ...
        view
    {
        ...
        if(p.order.isBuy()) {
            // If it's a BUY that means tokenB is the primary. That's how much we care about getting. Therefore there's
            // no need to scale the order based on spendableS.
            p.fillAmountB = p.order.amountB;
        } else {
            p.fillAmountB = p.fillAmountS.mul(p.order.amountB) / p.order.amountS;
        }

        // If relay sets max primary fill amount, re-balance max fill amounts if needed
...
// File: contracts/loopring/helper/RingHelper.sol
...
// File: contracts/loopring/impl/RingSubmitter.sol
...
//    function submitRings(
//        Data.SubmitRingsRequest memory request
//    ) public {
//        Data.Ring[] memory rings = ExchangeDeserializer.deserializeRingIndices(request.orders, request.ringIndices);
//        submitRings(request.mining, request.orders, rings);
//    }
        ...
        internal
    {
        ...
        // Update miner if it's set to the default value
        if (mining.miner == address(0x0)) {
            mining.miner = mining.feeRecipient;
        }

        if(!ITradeDelegate(delegateAddress).isTrustedSubmitter(tx.origin)) {
            // Only check the miner signature if the submitter isn't "trusted".
            mining.updateHash(rings);
            require(mining.checkMinerSignature(), INVALID_MINER_SIGNATURE);

            // We only need to check the dual auth signatures of submitters that aren't "trusted"
            for (i = 0; i < orders.length; i++) {
