...
contract AaveLiquidate is ReentrancyGuard, Ownable {
  ...
  function getMaxDebt(address _user) public returns (address _reserve, uint256 _amount) {
  ...
  function liquidate(address _collateral, address _reserve, address _user) public {
    uint256 _debt = getDebt(_collateral, _user);
    ...
    OneSplit(one).goodSwap(_collateral, _reserve, IERC20(_collateral).balanceOf(address(this)), 1, 1, 10);
