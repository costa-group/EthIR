...
contract UniSwap_ETH_CDAIZap is Initializable {
    ...
    function initialize() initializer public {
        ...
        DAI_TOKEN_ADDRESS = address(0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643);
    ...
    function Redeem(address _towhomtosend, uint256 _amount) public stopInEmergency returns (uint) {
        ...
        UniSwapExchangeContractAddress.transferFrom(msg.sender, address(this), _amount);
        ...
        (uint256 eth, uint256 tokens) = UniSwapExchangeContractAddress.removeLiquidity(_amount, ownerSharesEth, ownerSharesToken, SafeMath.add(now,1800));

        // Approve onsplit to take the compound tokens
        ...
        (bool result, ) = _towhomtosend.call.value(ethReturn)("");
    ...
    function getReturn(address _UniSwapExchangeContractAddress, IERC20 _ERC20TokenAddress, uint256 _value) public view returns (uint, uint, uint) {
        // ETH balance in uniswap contract
        uint256 ethBalance = address(_UniSwapExchangeContractAddress).balance;
        // Token balance in uniswap contract
        uint256 tokenBalance = _ERC20TokenAddress.balanceOf(_UniSwapExchangeContractAddress);

        // Get total pool shares
        uint256 totalShares = IuniswapExchange(_UniSwapExchangeContractAddress).totalSupply();

        // Calculate owner share pool
        uint256 ownerSharesEth = SafeMath.mul(SafeMath.div(_value, totalShares), ethBalance);
        uint256 ownerSharesToken = SafeMath.mul(SafeMath.div(_value, totalShares), tokenBalance);

        // Calculate eth value
        uint256 ethBought = IuniswapExchange(_UniSwapExchangeContractAddress).getTokenToEthInputPrice(ownerSharesToken);
        uint256 ethValue = SafeMath.add(ethBought, ownerSharesEth);
