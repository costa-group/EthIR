...
contract OfferToken is LexDAORole, ERC20Burnable, ERC20Capped {
    ...
    ***************/
    ...
    address payable private _owner; // controls OfferToken management functions - initial default to offeror
    ...
    bool public closed; // public status of offer closure / default = "false" / adjustable by offeror-owner
    ...
    ***************/
    ...
    constructor(
        ...
        _addLexDAO(_lexDAO); // add lexDAO governance role

        _mint(_offeror, _init); // mint offeror designated initial amount of OfferToken 

        _setupDecimals(_decimals); // set token decimals - fixed after deployment

        _transferOwnership(_offeror); // offeror is initial owner of OfferToken contract
    ...
    ***************/
    ...
    function buyOfferToken() public payable { // public can swap ether (Ξ) for OfferToken at registered rate
        ...
        _owner.transfer(msg.value); // forwards ether (Ξ) to offeror-owner 
    ...
    function writeRedemptionReview(uint256 rNumber, string memory review) public {
        ...
        require(rr.responded == true); // offeror-owner response must be provided prior to review
    ...
    ***************/
    ...
    function adjustOfferRate(uint256 newRate) public onlyOwner { // owner can adjust rate between ether (Ξ) and OfferToken
    ...
    function updateOfferStatus(bool offerClosed) public onlyOwner { // owner can open and close OfferToken market 
    ...
    function writeRedemptionResponse(uint256 rNumber, string memory response) public onlyOwner { // owner can record redemption responses
...
contract OfferTokenFactory is Context {
    ...
    constructor (address payable lexDAO, address payable owner, uint256 _factoryFee, string memory _stamp) public { 
    ...
    function newOfferToken(
        ...
        string memory _offer) public payable {
