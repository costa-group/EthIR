...
contract Billionaire is ownerShip {
    ...
    function regUser(uint _referrerID) public payable {
        ...
        if(MultiadminInterface(multiadminAddress).partner(msg.sender)==0)
    ...
    function buyLevel(uint _level) public payable {
        ...
        if(msg.sender!=ownerWallet && MultiadminInterface(multiadminAddress).partner(msg.sender)==0){
    ...
    function payForLevel(uint _level, address _user) internal {
...

        {
            ...
            emit paidForLevelEv(ownerWallet, msg.sender, _level, 0, now);
