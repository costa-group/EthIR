    ...
    ConstantAddresses
{
    ...
    function createStrategyDeposit(uint256 amount, uint256 flashLoanAmount, IDfDepositToken attachedToken) public onlyOwner {
    ...
    function addStrategyDeposit(uint256 amount, uint256 flashLoanAmount) public onlyOwner {
        require(token != IDfDepositToken(0x0));
        require(dfWallet != address(0x0));
        IToken(DAI_ADDRESS).transferFrom(msg.sender, address(this), amount);
    ...
    function closeStrategyDeposit(uint256 minUsdtForCompound, bytes memory data) public onlyOwner {
        require(dfWallet != address(0x0));
