...
contract USDNT is UtilUSDNT, WhitelistAdminRole {
        ...
        isHuman()
    {
        ...
        /*if (rInvestMoney[rid] != 0 && (rInvestMoney[rid].div(100000000).div(ethWei) != rInvestMoney[rid].add(value).div(100000000).div(ethWei))) {
            bool isEnough;
            uint sendMoney;
    ...
    function reInvestIn() public {
        ...
        /*if (rInvestMoney[rid] != 0 && (rInvestMoney[rid].div(100000000).div(ethWei) != rInvestMoney[rid].add(reInvestAmount).div(100000000).div(ethWei))) {
            (isEnough, sendMoney) = isEnoughBalance(rInfo[rid].luckPort);
            if (sendMoney > 0) {
                sendMoneyToUser(msg.sender, sendMoney);
            }
