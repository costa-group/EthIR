...
contract $USDAI is ERC20, ERC20Detailed {
    ...
    function mint$USDAI(uint256 amount) public {
        uint256 usdcSplit = amount.mul($USDCunit).div(2);
        uint256 daiSplit = amount.mul($DAIunit).div(2);
        $USDC.transferFrom(msg.sender, address(this), usdcSplit); // deposit $USDC split after approval
        $DAI.transferFrom(msg.sender, address(this), daiSplit); // deposit $DAI split after approval
        _mint(msg.sender, amount.mul($USDAIunit)); // mint $USDAI
    }
    
    function redeem$USDAI(uint256 amount) public {
        uint256 usdcSplit = amount.mul($USDCunit).div(2);
        uint256 daiSplit = amount.mul($DAIunit).div(2);
        $USDC.transfer(msg.sender, usdcSplit); // return $USDC split 
        $DAI.transfer(msg.sender, daiSplit); // return $DAI split 
        _burn(msg.sender, amount.mul($USDAIunit)); // burn $USDAI
    }
}