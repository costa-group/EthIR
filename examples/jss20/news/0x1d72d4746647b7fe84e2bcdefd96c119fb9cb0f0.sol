...
/////////////////////////////////////////// Mint ///////////////////////////////////////
     ...
     */
    function batchMint(address[] memory to, uint256[] memory ids, bytes32[] memory uris, uint256[] memory values) public onlyMinter {
        ...
        for (uint i = 0; i < ids.length; i++) {
            if (isNFT(ids[i]) && values[i] == 1) {
                _mintNonFungible(to[i], ids[i], uris[i]);
     ...
     */
    function mintNonFungible(address to, uint256 tokenId, bytes32 byteUri) external onlyMinter {
     ...
     * @param byteUri bytes32 Concatenated metadata URI of nft to be minted
     */
    function _mintNonFungible(address to, uint256 id, bytes32 byteUri) internal {
        require(to != address(0x0));
        require(!exists(id));
