...
contract CompoundSaverProxy is ExchangeHelper, DSMath {
    ...
    function repay(
        ...
        uint maxColl = getMaxCollateral(_addrData[0], user);
    ...
    function boost(
        ...
        uint maxBorrow = getMaxBorrow(_addrData[1], user);
    ...
    function getMaxCollateral(address _cCollAddress, address _user) public returns (uint) {
        (, uint liquidityInEth, ) = ComptrollerInterface(COMPTROLLER).getAccountLiquidity(_user);
        uint usersBalance = CTokenInterface(_cCollAddress).balanceOfUnderlying(_user);
    ...
    function getMaxBorrow(address _cBorrowAddress, address _user) public view returns (uint) {
        (, uint liquidityInEth, ) = ComptrollerInterface(COMPTROLLER).getAccountLiquidity(_user);
