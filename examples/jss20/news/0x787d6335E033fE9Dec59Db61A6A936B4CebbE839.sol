...
// File: contracts/SatoshiToken.sol
 ...
 */
contract SatoshiToken is AccessControl, ERC20Burnable, ERC20Pausable, GSNRecipientSignature {
    ...
    // Maximum total supply of Satoshi tokens i.e. 21 million max
    uint256 public constant MAX_TOKEN_SUPPLY = 21000000 ether;

    /**
     * @dev Initialize the contract with token name, symbol, decimals and truster signer.
     *
     * @param _name - Name of the token
     * @param _symbol - Symbol of the token
     * @param _decimals - number of decimals digits for the token
    ...
    function pause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "SatoshiToken: must have pauser role to pause");
        super.pause();
    }
    ...
    function unpause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "SatoshiToken: must have pauser role to unpause");
        super.unpause();
    }
     ...
     */
    function burnFrom(address account, uint256 amount) public {
        require(hasRole(BURNER_ROLE, _msgSender()), "SatoshiToken: must have burner role to burnFrom");
    ...
    function grantMinterRole(address account) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "SatoshiToken: sender must be an admin to grant");
    ...
    function grantPauserRole(address account) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "SatoshiToken: sender must be an admin to grant");
        super.grantRole(PAUSER_ROLE, account);
