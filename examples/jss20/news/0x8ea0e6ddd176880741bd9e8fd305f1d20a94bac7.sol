...
// File: contracts/RandomWorld.sol
...
/**
 * @title RandomWorld is an example contract which requests data from
...
contract RandomWorld is ChainlinkClient, Ownable {
  ...
  // NEED TO MODIFY BEFORE DEPLOY TO MAINNET
   ...
   * @param _modulo The modulo indicates which random factor was chosen
   * @param _choice The choice indicates user's choice
  ...
  function placeBet(uint _modulo, uint _choice, address _oracle, bytes32 _jobId) external payable {
    ...
    // start to request a random number
  ...
  /**
   * @notice The closeBet to close specific bet with requeistId
  ...
  /**
   * @notice could be invoked by anyone
   * @dev The refundBet function to refund expired bet
   * @param _requestId The ID that was generated for the request
   */
  ...
  /**
   * @dev increase balance to let more people participant
   */
