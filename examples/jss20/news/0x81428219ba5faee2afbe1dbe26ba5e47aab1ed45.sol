...
interface IGodsUnchained {
    ...
    /// @dev Returns the level for a particular board given its token id
    /// @param tokenId The id of the board to query.
    /// @return proto The proto of the given board.
    /// @return level The quality of the given board
    function getBoardLevel(uint256 tokenId)
        external
        view
        returns (uint8 level);
}
    ...
    IPropertyValidator
{
    ...
    /// @dev Checks that the given board (encoded as assetData) has the level encoded in `propertyData`.
    ///      Reverts if the board doesn't match the specified level.
    /// @param tokenId The ERC721 tokenId of the board to check.
    /// @param propertyData Encoded level that the board is expected to have.
    function checkBrokerAsset(
        uint256 tokenId,
        bytes calldata propertyData
    )
        ...
        view
    {
        ...
        (uint8 level) = GODS_UNCHAINED.getBoardLevel(tokenId);
