...
contract yCurveExchange is ReentrancyGuard, Ownable {
  ...
  function calc_deposit_amount(uint256[5] calldata amounts) external view returns (uint256) {
    uint256[4] memory _y;
    _y[0] = amounts[0].mul(1e8).div(yERC20(yDAI).getPricePerFullShare());
    _y[1] = amounts[1].mul(1e8).div(yERC20(yUSDC).getPricePerFullShare());
    _y[2] = amounts[2].mul(1e8).div(yERC20(yUSDT).getPricePerFullShare());
    _y[3] = amounts[3].mul(1e8).div(yERC20(yTUSD).getPricePerFullShare());
    uint256 _y_output = yCurveFi(ySWAP).calc_token_amount(_y, true);
    uint256[2] memory _s;
    _s[0] = amounts[4].mul(1e8).div(yERC20(ySUSD).getPricePerFullShare());
    _s[1] = _y_output;
