...
/**
 * @title DharmaSmartWalletImplementationV7 (prototype 0, staging version)
 * @author 0age
 ...
 */
contract DharmaSmartWalletImplementationV7Prototype0Staging is
  ...
  DTokenInterface internal constant _DDAI = DTokenInterface(
    0xD63fc503C0086c812cE5d97757e4652Cf06d365b // mainnet (prototype 0)
  );

  DTokenInterface internal constant _DUSDC = DTokenInterface(
    0xdDCe908d94d07eCf607Cc1992E27C1d33Bfb5f1c // mainnet (prototype 0)
  );
   ...
   * approval on the dDai and dUSDC contracts, and deposit any Dai or USDC
  ...
  function _withdrawUSDCAtomic(
    ...
    bool maxWithdraw = (amount == uint256(-1));
    if (maxWithdraw) {
      // First attempt to redeem all dUSDC from Compound if there is a balance.
   ...
   * directly and held in Dharma Dai and Dharma USD Coin. This is not a view
   * function since Compound will calculate accrued interest as part of the
   * underlying balance checks on each Dharma Token, but can still be called
   * from an off-chain source as though it were a view function.
  ...
  /**
   * @notice Redeem all available cToken for the underlying and use that to mint
   * the respective dToken. If any step in the process fails, the call will
   * revert and prior steps will be rolled back. Also note that existing
   * underlying tokens are not included as part of this operation.
   */
  function _migrateCTokenToDToken(AssetType token) internal {
    ...
    // Only perform the call to redeem if there is a non-zero balance.
    if (redeemAmount > 0) {
      // Get the current underlying balance on this contract.
      uint256 currentBalance = underlying.balanceOf(address(this));

      // Redeem underlying balance from the cToken and revert if unsuccessful.
      require(
        cToken.redeem(redeemAmount) == _COMPOUND_SUCCESS, "cToken redeem failed."
      );

      // Calculate difference between pre-redeem and post-redeem balances.
      uint256 balance = cToken.balanceOf(address(this)) - currentBalance;
      
      // If the allowance is insufficient, set it before depositing.
      if (underlying.allowance(address(this), address(dToken)) < balance) {
        require(
          underlying.approve(address(dToken), uint256(-1)), "Approval failed."
        );
      }
