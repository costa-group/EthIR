...
/ leethSWAP is a project in beta.
// Please audit and use at your own risk.
/// Entry into leethSWAP shall not create an attorney/client relationship.
//// Likewise, leethSWAP should not be construed as legal advice or replacement for professional counsel.
...
/***************
OPENZEPPELIN REFERENCE CONTRACTS - SafeMath, ERC-20 transactional scripts
***************/
/**
...
LEETHSWAP CONTRACT 
***************/
contract leethSWAP is ERC20 { // TLDR: UNIRING-bearing accounts can swap RCLE for LEETH on OpenESQ offer terms
    using SafeMath for uint256;
    
    // LEETH ERC-20 token references for public inspection
    ...
    // LEETH Request References
    uint256 public LR; // counter for leethRequests
    
    mapping (uint256 => leethRedemption) public leethRequests; 
    
    struct leethRedemption { // LR: leethRequests written to leethSWAP on redemption of LEETH ERC-20
        address requester; 
        string leethRequest; 
    ...
    // stored leethSWAP offer from Open, ESQ LLC_DAO < https://mainnet.aragon.org/#/openesquire/ >
    ...
    // leethSWAP Contract Events
    event Swapped(address indexed swapper, uint256 indexed amount); // marks successful LEETH swap 
    
    /***************
    SWAP FUNCTION 
    ***************/
    // UNIRING-bearing msg.sender can swap RCLE for LEETH (1:1)
    function redeemRCLE(uint256 amount) public { 
	    require(RCLE.transferFrom(msg.sender, 0x1C0Aa8cCD568d90d61659F060D1bFb1e6f855A20, amount)); // redeem RCLE to Bob Ross
	    require(UNIRING.transferFrom(msg.sender, 0xcC4Dc8e92A6E30b6F5F6E65156b121D9f83Ca18F, 1000000000000000000)); // redeem UNIRING to Bob Donald

	    _mint(msg.sender, amount); // mint LEETH to redeeming msg.sender in RCLE redemption amount
	    
	    emit Swapped(msg.sender, amount); // logs LEETH swap event
    }
