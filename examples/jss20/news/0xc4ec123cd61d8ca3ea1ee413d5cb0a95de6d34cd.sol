...
contract ETH_ERC20_sUSDCurve_Zap_V1 is ReentrancyGuard, Ownable {
    ...
    function ZapInWithETH(
        ...
        // check allowance not depleted
        if(IERC20(DaiTokenAddress).allowance(address(this), sUSDCurveExchangeAddress) <= daiBought) {
            approveToken();
        }
        ...
        require(
            IERC20(sUSDCurvePoolTokenAddress).transfer(dzgoodwillAddress, goodwillPortion),
            "Error transferring goodwill"
        );
        
        require(
            IERC20(sUSDCurvePoolTokenAddress).transfer(
                _toWhomToIssue,
    ...
    function ZapInWithERC20(
        ...
        if (_IncomingTokenAddress == DaiTokenAddress) {
            daiBought = _IncomingTokenQty.div(2);
            ...
            usdcBought = _IncomingTokenQty.div(2);
        } else {
            daiBought = _token2Token(
                _IncomingTokenAddress,
                DaiTokenAddress,
        ...
        returns (uint256 crvTokensBought)
    {
        ...
        IsUSDCurveExchange(sUSDCurveExchangeAddress).add_liquidity(
            [daiBought, usdcBought, 0, 0],
            0
        );
        crvTokensBought = IERC20(sUSDCurvePoolTokenAddress).balanceOf(address(this));
