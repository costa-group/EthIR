...
interface DTokenInterface {
  ...
  function mintViaCToken(uint256 cDaiToSupply) external returns (uint256 dDaiMinted);

  function redeem(uint256 dTokenToBurn) external returns (uint256 underlyingReceived);

  function redeemToCToken(uint256 dDaiToBurn) external returns (uint256 cDaiReceived);

  function redeemUnderlying(uint256 underelyingToReceive) external returns (uint256 dTokensBurned);

  function redeemUnderlyingToCToken(uint256 daiToReceive) external returns (uint256 dDaiBurned);
...
contract DharmaDaiPrototype1 is ERC20Interface, DTokenInterface {
   ...
   */
  function getSurplus() external view returns (uint256 daiSurplus) {
  ...
  function _getAccruedInterest() internal view returns (
    ...
    if (fullyAccrued) {
      ...
      uint256 spread = _SPREAD.getDaiSpreadPerBlock().add(_SCALING_FACTOR);
      ...
      // All interest is taken by the spread if it exceeds the interest on cDai.
      if (spreadInterest >= cDaiInterest) {
        dDaiExchangeRate = _dDaiExchangeRate;
      } else {
        // Use total cDai interest, less the spread, to set dDai exchange rate.
        dDaiExchangeRate = _dDaiExchangeRate.mul(
          cDaiInterest - spreadInterest
        ) / _SCALING_FACTOR;
      }
    }
  }
