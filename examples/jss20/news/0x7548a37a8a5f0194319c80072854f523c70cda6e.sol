...
contract yCRV is ERC20, ERC20Detailed, ReentrancyGuard, Ownable {
      ...
      nonReentrant
  {
      ...
      supplyCompound(b);
      ICurveFi(curve).add_liquidity([b,0],now.add(1800));
      pool = calcPoolValueInToken();
  }
  ...
  function calcPoolValueInToken() public view returns (uint) {
    ...
    return price.mul(supply).div(1e18);
  }
  ...
  function withdraw(uint256 _amount) internal {
    ...
    uint256 compoundDai = dai.mul(1e16).div(Compound(cDAI).exchangeRateStored()).mul(105).div(100);
