...
contract DssProxyHelpers is Common {
    ...
    event LogDrawSend(uint vaultNum, uint daiAmt, address to, address owner);
    event LogOpenLockDraw(uint vaultNum, uint collAmt, uint daiAmt, address collJoin, address owner);
    event LogWipeFreeShut(uint vaultNum, address collJoin, address owner);
    ...
    function give(uint cdp, address usr) internal {
    ...
    function move(uint cdp, address dst, uint rad) internal {
        address manager = InstaMcdAddress(getMcdAddresses()).manager();
...
contract DssProxyActionsAdv is DssProxyHelpers {
    ...
    function lockETHAndDraw(uint cdp, address ethJoin, uint wadD) public payable {
        address manager = InstaMcdAddress(getMcdAddresses()).manager();
        address jug = InstaMcdAddress(getMcdAddresses()).jug();
        address daiJoin = InstaMcdAddress(getMcdAddresses()).daiJoin();
        address urn = ManagerLike(manager).urns(cdp);
    ...
    function openLockETHAndDraw(address ethJoin, bytes32 ilk, uint wadD) public payable returns (uint cdp) {
        cdp = open(ilk, address(this));
        lockETHAndDraw(cdp, ethJoin, wadD);
        emit LogOpenLockDraw(
            cdp,
            msg.value,
            wadD,
            ethJoin,
    ...
    function wipeAllAndFreeEth(uint cdp, address ethJoin) public {
        address manager = InstaMcdAddress(getMcdAddresses()).manager();
        address vat = ManagerLike(manager).vat();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
    ...
    function wipeFreeEthAndShut(uint cdp, address ethJoin) public {
        wipeAllAndFreeEth(cdp, ethJoin);
        shut(cdp);
