...
contract IDAToken is ERC20Capped, ERC20Burnable, ERC1363, Roles, TokenRecover {
    ...
    modifier canMint() {
        require(!_mintingFinished, "IDAtoken: minting is finished");
        _;    }

    modifier canTransfer(address from) {
        require(
            _transferEnabled || hasRole(OPERATOR_ROLE, from),
            "IDAToken: transfer is not enabled or from does not have the OPERATOR role"
        );
        _;
    }
        ...
        ERC1363(name, symbol)
    {
        require(
            ...
            "IDAToken: if finish minting, cap must be equal to initialSupply"
        );
