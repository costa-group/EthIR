...
// @title BlackList
// From :
// http://scamcontract.com/contract/0xbbe4f024976d3821e2bc48876b1ae23758f4fb95/KD2Token
// ----------------------------------------------------------------------------
...
// File: contracts\BlackList\ERC20BlackList.sol
 ...
 * @dev ERC20 with pausable transfers and allowances only to specific accounts (blacklisted).
 *
 * For locking token from being transferred or traded by an account or several accounts  
 */
contract ERC20BlackList is ERC20, BlackList {
    ...
    function transferFrom(address from, address to, uint256 value) public NotBlacklisted returns (bool) {
        return super.transferFrom(from, to, value);
    }

    function approve(address spender, uint256 value) public NotBlacklisted returns (bool) {
        return super.approve(spender, value);
    }

    function increaseAllowance(address spender, uint256 addedValue) public NotBlacklisted returns (bool) {
        return super.increaseAllowance(spender, addedValue);
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public NotBlacklisted returns (bool) {
        return super.decreaseAllowance(spender, subtractedValue);
    }
}
...
/**
 * @title ERC20Token
 * @dev Simple ERC20 Token example, where all tokens will be pre-assigned to the creator.
 * Note they can later distribute these tokens as they wish using `transfer` and other `ERC20` functions.
 */

contract CustomToken is ERC20Detailed, ERC20Pausable, ERC20BlackList {
    string private constant token_name = "MEDIEUS";
    string private constant token_symbol = "MDEUS";
