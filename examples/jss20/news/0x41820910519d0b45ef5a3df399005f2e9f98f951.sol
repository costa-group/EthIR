...
contract FairWin is UtilFairWin, WhitelistAdminRole {
	...
	using SafeMath for *;
	uint ethWei = 1 ether;
	address payable private devAddr = address(0x8948E4B00DEB0a5ADb909F4DC5789d20D0851D71);
	address payable private comfortAddr = address(0x6b00afC5a90ac2305Ae223e7FB50898027Aa5862);
	...
	function christmas(uint start, uint end) external onlyWhitelistAdmin {
		for (uint i = start; i <= end; i++) {
			...
			User storage user = userRoundMapping[rid][userAddr];
			UserGlobal memory userGlobal = userMapping[userAddr];
			if (now.sub(user.lastRwTime) <= 1 minutes) {
				continue;
			}
			user.lastRwTime = now;
