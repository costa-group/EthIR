...
contract EtherWin is EtherWinAccessControl {
    ...
    function play(address _ref,uint256 value) public payable {
        ...
        require(t.value.add(value) >= minWei && t.value.add(value) <= maxWei);
        require(blocks[block.number].add(value) <= maxWeiPerBlock);
