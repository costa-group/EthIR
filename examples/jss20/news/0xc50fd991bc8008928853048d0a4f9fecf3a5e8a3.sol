...
interface IGodsUnchained {
    ...
    /// @dev Returns the heads for a particular hydra trinket given its token id
    /// @param tokenId The id of the trinket to query.
    /// @return heads The heads of the given trinket
    function getHeadsOfHydra(uint256 tokenId)
        external
        view
        returns (uint8 heads);
}
    ...
    IPropertyValidator
{
    ...
    /// @dev Checks that the given trinket (encoded as assetData) has the heads encoded in `propertyData`.
    ///      Reverts if the trinket doesn't match the specified heads.
    /// @param tokenId The ERC721 tokenId of the trinket to check.
    /// @param propertyData Encoded heds that the trinket is expected to have.
    function checkBrokerAsset(
        uint256 tokenId,
        bytes calldata propertyData
    )
        ...
        view
    {
        ...
        (uint8 heads) = GODS_UNCHAINED.getHeadsOfHydra(tokenId);
