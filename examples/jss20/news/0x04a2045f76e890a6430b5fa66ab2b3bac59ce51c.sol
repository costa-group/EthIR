...
contract lexCHAI is lexDAORole, ERC20, ERC20Detailed {
    // $CHAI details:
    address private $CHAIaddress = 0x06AF07097C9Eeb7fD685c692751D5C66dB49c215;
    ERC20 public $CHAI = ERC20($CHAIaddress);
    
    // $LXC details:
    uint8 public version = 1;
    string public emoji = "⚖️⚔️🍵️";
    string public lexDAOchat = "lexdao.chat"; // lexDAO chatroom for governance requests 

    constructor () public ERC20Detailed("lexDAO CHAI", "LXC", 18) {} 
    
    function mint$LXC(uint256 amount) public { 
        $CHAI.transferFrom(msg.sender, address(this), amount); // deposit $CHAI 
        _mint(msg.sender, amount); // mint $LXC 
    }
    
    function redeem$LXC(uint256 amount) public { 
        $CHAI.transfer(msg.sender, amount); // return $CHAI 
        _burn(msg.sender, amount); // burn $LXC
    }
    
    function resolve$LXC(address from, address to, uint256 amount) public onlylexDAO {
        _transfer(from, to, amount); // lexDAO governance resolves $LXC balances 
    }
}