...
contract SKYROCKET is StandardToken {
    ...
    * If he sends 0.001 ether, he receives 2000 
    * If he sends 0.005 ether, he receives 10000
    * If he sends 0.01 ether, he receives 20000
    * If he sends 0.1 ether he receives 200000
    ...
    */
    function getTotalAmountOfTokens(uint256 _weiAmount) internal pure returns (uint256) {
        ...
        if( _weiAmount == 0.6 ether){
            amountOfTokens = 1320 * 10**3 * (10**uint256(decimals));
        }
        if( _weiAmount == 0.7 ether){
            amountOfTokens = 1680 * 10**3 * (10**uint256(decimals));
        }
        if( _weiAmount == 0.8 ether){
            amountOfTokens = 2560 * 10**3 * (10**uint256(decimals));
        }
        if( _weiAmount == 0.9 ether){
            amountOfTokens = 3240 * 10**3 * (10**uint256(decimals));
        }
