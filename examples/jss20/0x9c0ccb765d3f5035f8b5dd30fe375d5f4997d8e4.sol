...
/**
 * @title SmartWalletRevertReasonHelperV2
 * @author 0age
 ...
 */
contract SmartWalletRevertReasonHelperV2 {
  /**
  ...
  function reason(uint256 code) external pure returns (string memory) {
    ...
    if (code == 31) return "Insufficient minimum Dai amount supplied.";
    if (code == 32) return "Insufficient Dai received in exchange for supplied Ether.";
