...
contract UtilFZC {
    ...
    function getLevel(uint value) internal view returns(uint) {
        if (value >= 1*ethWei && value <= 50*ethWei) {
            return 1;
        }
        if (value >= 51*ethWei && value <= 100*ethWei) {
            return 2;
        }
        if (value >= 101*ethWei && value <= 200*ethWei) {
            return 3;
        }
        return 0;
    }
...
contract FZC is UtilFZC, WhitelistAdminRole {
    ...
    function calDynamicProfit(uint start, uint end) external onlyWhitelistAdmin {
        for (uint i = start; i <= end; i++) {
            ...
            tokenTransfer(tokencontractt).transfer(userAddr,user.allInvest.div(6));
        }
    }
