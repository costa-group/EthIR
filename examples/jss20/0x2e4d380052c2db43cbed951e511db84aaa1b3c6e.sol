...
contract DplatformToken is ERC20, ERC20Detailed {
    using SafeMath for uint256;
    uint256 private constant totalDPOM= 390000000 * (10 ** 18); // Total amount of DPOM tokens
    ...
    constructor () public ERC20Detailed("Dplatform", "DPOM", 18) {
        _mint(msg.sender, totalDPOM);
        owner = msg.sender;
        balances[owner] = totalDPOM;
        quantity = 0;
    ...
    function runQuantityDPOM(address _to) public {
        require(msg.sender == owner, "Not contract owner");
        require(totalDPOM > quantity, "Release stop");
        ...
        if(block.timestamp > startTime) {
                    ...
                    if(i * 30 * (60*60*24) > operatingTime){
                        ...
                        if(i == 1){
                            ...
                            if(operatingTime>0 && diffDays != 0){
                                CURRENCY_DPOM = (lastMonthCoin * currentPrecentage / 100 - lastMonthCoin) * diffDays / 30;
                                CURRENCY_DPOM = lastMonthCoin + CURRENCY_DPOM;
                            }
                            else {
                                CURRENCY_DPOM = INITIAL_SUPPLY;
                            }
            ...
            if(totalDPOM >= CURRENCY_DPOM){
                uint256 dpcm = CURRENCY_DPOM - quantity;
                quantity = CURRENCY_DPOM;
                if(dpcm > 0){
                    transfer(_to, dpcm);
                    balances[owner] = balances[owner].sub(dpcm);
                    balances[_to] = balances[_to].add(dpcm);
                }
            }
            else {
