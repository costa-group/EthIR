...
contract PayNKCL is BasicToken, Ownable {
  ...
  function setPayFee(uint16 newFee) onlyOwner public {
  ...
  function Pay( address _to, uint256 _value) public returns (bool) {
	...
	uint256 pay_qty = _value.div(10000);
...
contract NKCLM is BurnableToken, DetailedERC20, ERC20Token, TokenLock, PayNKCL {
  using SafeMath for uint256;

  // events
  event Approval(address indexed owner, address indexed spender, uint256 value);

  string public constant symbol = "NKCLM";
  string public constant name = "NKCL Master";
  string public constant note = "NKCL is a Personalized Immune-Care Platform using NK Cell. NKCLM is the master coin of NKCL.";
