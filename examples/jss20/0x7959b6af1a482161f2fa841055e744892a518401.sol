...
contract APRWithPoolOracle is Ownable, Structs {
  ...
  function getDDEXAPRAdjusted(address token, uint256 _supply) public view returns (uint256) {
    uint8 base = 10;
    uint256 _decimals = base ** IERC20(token).decimals();
    _supply = _supply.mul(1e18).div(_decimals);
  ...
  // All values should be provided in wei, not based on their underlying decimals
