...
contract LAOtoken is Restrictable, Whitelistable, ERC1404, FundsDistributionToken {
		...
		external 
	{
		...
		require(fundsToken.transfer(msg.sender, withdrawableFunds), "LAOtoken: TRANSFER_FAILED");
	 ...
	 * @return An int256 representing the difference of the new and previous funds token balance
	 */
...
contract LAOtokenFactory {
    using SafeMath for uint256;
    
    // Factory stamp message
    string public stamp;
    
    // Factory ether (Ξ) fee ~ manager might, e.g., secure stamp data of LT Issuances for this value
    uint256 public etherFee;
    
    LAOtoken private LT;
    
    // Receives and updates ether (Ξ) fee
    ...
    function newLAOtoken( // public can issue stamped LT for ether (Ξ) fee
        string memory name, 
		string memory symbol,
		string memory _stamp,
		uint8 decimals,
