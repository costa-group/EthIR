...
contract OneHelpers is Helpers {
    ...
    function getExpectedReturn(
        ...
        uint sellAmt
    ) internal view returns (uint buyAmt, uint[] memory distribution){
...
contract BasicResolver is OneHelpers {
    ...
    function sell(
        ...
        uint ethAmt = sellAddr == getAddressETH() ?  _sellAmt : 0;

        uint _slippageAmt = convert18ToDec(_buyAddr.decimals(), wmul(unitAmt, convertTo18(_sellAddr.decimals(), _sellAmt)));
        {
        (uint buyAmt, uint[] memory distribution) = getExpectedReturn(
            oneSplitContract,
            _sellAddr,
            _buyAddr,
            _sellAmt
