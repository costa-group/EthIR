...
contract MKRSwapper is  Helpers {
    ...
    function getBestMkrSwap(address srcTknAddr, uint destMkrAmt) public view returns(uint bestEx, uint srcAmt) {
        if (srcTknAddr == getDaiAddress()) { //Check samyak - oasis was failing.
            srcAmt = getUniswapSwap(srcTknAddr, destMkrAmt);
            bestEx = 1;
        } else {
...
contract MCDResolver is SCDResolver {
    ...
    function getDaiDebt(uint cdp) internal returns (uint debt) {
        address manager = InstaMcdAddress(getMcdAddresses()).manager();
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = 0x4554482d41000000000000000000000000000000000000000000000000000000;
        (, uint art) = VatLike(ManagerLike(manager).vat()).urns(ilk, urn);
...
contract MigrateHelper is MCDResolver {
    ...
    ) internal returns (uint _wad, uint _ink, uint maxConvert)
    {
        ...
        if (payFeeWith == getDaiAddress() || payFeeWith == getSaiAddress()) { //Check samyak
    ...
    ) internal
    {
        ...
        if (payFeeWith == getDaiAddress() || payFeeWith == getSaiAddress()) {
            (,uint tknAmt) = getBestMkrSwap(payFeeWith, getFeeOfCdp(scdCup, _wad));
...
contract MigrateResolver is MigrateHelper {
    ...
    ) external payable
    {
        ...
        require(add(getDaiDebt(mcdCdp), _wad) >= 20*(10**18), "Min 20 Dai required to migrate.");
