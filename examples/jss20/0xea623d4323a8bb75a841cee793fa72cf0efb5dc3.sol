...
// File: contracts\interfaces\IDfFinanceDeposits.sol
...
// File: contracts\deposits\interfaces\IDfDepositToken.sol
...
interface IDfDepositToken {
    ...
    function transferBackLockedTokens(address[] calldata accounts, address target) external;
    function transferWithLock(
        address sender, address recipient, uint256 amount, uint256 timestampDivs
...
// File: contracts\deposits\DfTokenizedDeposit.sol
...
//    IDfFinanceDeposits public constant dfFinanceDeposits = IDfFinanceDeposits(0xCa0648C5b4Cea7D185E09FCc932F5B0179c95F17); // Kovan
    ...
    function createStrategyDeposit(uint256 amount, uint256 flashLoanAmount, IDfDepositToken attachedToken) onlyOwner public  {
    ...
    function transferBackLockedTokens(address[] memory accounts) onlyOwnerOrAdmin public {
        token.transferBackLockedTokens(accounts, owner);
    }
    function addStrategyDeposit(uint256 amount, uint256 flashLoanAmount) onlyOwner public  {
        require(token != IDfDepositToken(0x0));
        require(dfWallet != address(0x0));
        IToken(DAI_ADDRESS).transferFrom(msg.sender, address(this), amount);
    ...
    function closeStrategyDeposit(uint256 minUsdtForCompound, bytes memory data) onlyOwner public  {
        require(dfWallet != address(0x0));
    ...
    function calcUserProfit(address userAddress, uint256 max) public view returns(uint256 totalCompProfit, uint256 totalUsdtProfit, uint64 index) {
...
//            emit Profit(msg.sender, index, uint64(profitDai / 1e12), uint64(profitComp / 1e12));
        }
    }

    function transferWithLock(address recipient, uint256 amount, uint256 timestampDivs) public onlyOwner returns (bool) {
        return token.transferWithLock(msg.sender, recipient, amount, timestampDivs);
    }
    ...
    function userClaimProfitOptimized(uint64 lastIndex, uint256 totalUsdtProfit, uint256 totalCompProfit, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 hash = sha256(abi.encodePacked(this, msg.sender, lastIndex, totalUsdtProfit, totalCompProfit));
    ...
    function adminClaimProfit(uint256 minUsdtForCompound, bytes memory data) onlyOwnerOrAdmin public  {
