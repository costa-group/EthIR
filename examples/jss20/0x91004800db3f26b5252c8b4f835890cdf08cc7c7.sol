...
contract CurveFlash is ReentrancyGuard, Ownable, Structs {
  ...
  function callFunction(
    ...
    OneSplit(one).goodSwap(dai, usdt, IERC20(dai).balanceOf(address(this)), 1, 6, 0);
    OneSplit(one).goodSwap(usdt, usdc, IERC20(usdt).balanceOf(address(this)), 1, 6, 0);
    OneSplit(one).goodSwap(usdc, dai, IERC20(usdc).balanceOf(address(this)), 1, 6, 0);
  }
