...
contract ERC20SalaryFactory{
  event NewERC20SalaryFactory(address addr);
  function createERC20Salary(string memory name, address erc20bank, address multisig) public returns(address){
    ERC20Salary salary = new ERC20Salary(name, msg.sender, erc20bank, multisig);
    emit NewERC20SalaryFactory(address(salary));
