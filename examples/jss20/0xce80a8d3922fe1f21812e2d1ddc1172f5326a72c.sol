...
contract CoinGhostToken is ERC20 {
...
contract CoinGhost is ERC20 {
    string public constant name = "CoinGhost"; // solium-disable-line uppercase
    string public constant symbol = "GST"; // solium-disable-line uppercase
    uint8 public constant decimals = 18; // solium-disable-line uppercase
    ...
    function makeProposal(address target, uint8 mode, uint256 amount, address token) public onlyOwner {
        proposals[target] = ProposalInfo(mode, amount, CoinGhostToken(token));
        for (uint i=0; i < stakeHolders.length; i++){
 ...
 /**
   ...
   */
    ...
    */
    ...
    function burn(address _who, uint256 _value, CoinGhostToken ct) internal {
        require(_value <= super.balanceOf(_who), "Balance is too small.");
        require(tokenWallet != address(0), "token wallet is not set");
