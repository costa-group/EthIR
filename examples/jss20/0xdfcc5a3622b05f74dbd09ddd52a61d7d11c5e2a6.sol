...
contract CurveFlash is ReentrancyGuard, Ownable, Structs {
  ...
  function swapDAItoUSDC(uint256 amount) public {
    withdrawDAI(amount);
    ICurveFi(swap).exchange_underlying(0, 1, amount, 0);
    supplyUSDC(amount);
  }
  function swapUSDCtoDAI(uint256 amount) public {
    withdrawUSDC(amount);
    ICurveFi(swap).exchange_underlying(1, 0, amount, 0);
    supplyDAI(amount);
  }

  function supplyDAI(uint256 amount) public returns(uint) {
  ...
  function supplyUSDC(uint256 amount) public returns(uint) {
      Info[] memory infos = new Info[](1);
      infos[0] = Info(address(this), 0);
  ...
  function withdrawDAI(uint256 amount) internal {
      Info[] memory infos = new Info[](1);
      infos[0] = Info(address(this), 0);
