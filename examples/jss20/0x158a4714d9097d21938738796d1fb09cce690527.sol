...
// File: contracts/labor-contract/LaborContractDisputable.sol
...
/// @title Part of LaborContract functionality that provides dispute implementation.
/// @dev Should be used as a library extension
contract LaborContractDisputable is
	LaborContractBaseDisputable,
    InitializableOwned,
    LaborContractAbstract
{

    /// @dev Indices for contract dispute resolvement details values

    uint constant internal DISPUTE_RESOLVABLE_DETAILS_EMPLOYER_AMOUNT = 0;
    uint constant internal DISPUTE_RESOLVABLE_DETAILS_WITHDRAW_FEE_AMOUNT = 1;
    uint constant internal DISPUTE_RESOLVABLE_DETAILS_LENGTH = 2;
    ...
    function getResolvingContractDetails(bytes32 _contractId, uint _assigneeValue) external view returns (uint[] _values) {
        _values = new uint[](DISPUTE_RESOLVABLE_DETAILS_LENGTH);

        DataParser.ContractData memory _disputeContractInfo = DataParser.parseContractDataCleanInput(_getContractInfoBytes(_contractId));
        DataParser.ContractData memory _linkedContractInfo = DataParser.parseContractDataCleanInput(_getContractInfoBytes(_disputeContractInfo.linkedContractId));

        EscrowBaseInterface _escrow = getEscrow(_disputeContractInfo.linkedContractId);
        (, uint _escrowBalance) = _escrow.getBalanceOf(_disputeContractInfo.linkedContractId, _linkedContractInfo.employer, _linkedContractInfo.assignee);
        _values[DISPUTE_RESOLVABLE_DETAILS_EMPLOYER_AMOUNT] = _escrowBalance.sub(_assigneeValue);

        (uint _percent, uint _percentPrecision) = getServiceSurchargePercent(_disputeContractInfo.linkedContractId);
        _values[DISPUTE_RESOLVABLE_DETAILS_WITHDRAW_FEE_AMOUNT] = PercentCalculator.getPercent(_assigneeValue, _percent, _percentPrecision);
    }
    ...
    {
        DataParser.ContractData memory _disputeContractInfo = DataParser.parseContractDataCleanInput(_getContractInfoBytes(_contractId));
        require(_disputeContractInfo.contractType == uint8(DataParser.ContractType.DISPUTE), "C_CD");

        DataParser.ContractData memory _linkedContractInfo = DataParser.parseContractDataCleanInput(_getContractInfoBytes(_disputeContractInfo.linkedContractId));
        _resolveDisputeToEscrow(_disputeContractInfo.linkedContractId, _linkedContractInfo, _assigneeValue, _expireAtBlock, _signature);
        _setContractStateTo(_disputeContractInfo.linkedContractId, State.TERMINATED);
        _setContractStateTo(_contractId, State.COMPLETED);

        emit ContractDisputeResolved(_contractId);
        return OK;
    }

	/** PRIVATE */

    function _resolveDisputeToEscrow(
        bytes32 _contractId,
        DataParser.ContractData memory _contractInfo,
        uint _assigneeValue,
        uint _expireAtBlock,
    ...
    private
    {
        ...
        require(
            OK == _escrow.releaseDisputedPayment(
                _contractId,
