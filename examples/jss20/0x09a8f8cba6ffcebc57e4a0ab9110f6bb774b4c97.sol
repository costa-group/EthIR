 ...
 * and is backed by that stablecoin's respective Compound cToken. The V0 dToken
 ...
 * unimplemented internal functions with implementation-specific ones. V0 is a
 * fallback implementation, used in case of emergency, that will pause minting
 * and pulling surplus cTokens - redeeming and transferring dTokens will remain
 * unpaused so that users can still exit if desired.
 */
contract DharmaTokenV0 is ERC20Interface, DTokenInterface, DharmaTokenHelpers {
  ...
  /**
   * @notice Minting is paused in V0.
   */
  function mint(uint256) external returns (uint256) {
    // Minting is paused in V0.
    revert("Minting is paused in V0.");
  }

  /**
   * @notice Minting via cTokens is paused in V0.
   */
  ...
  /**
   * @notice Pulling surplus cTokens is paused in V0.
   */
  function pullSurplus() external returns (uint256) {
    // Pulling surplus is paused in V0.
 ...
 * derived from Open Zeppelin's ERC20 contract)
 * @notice This contract provides the V0 implementation of Dharma Dai (or dDai),
 * an upgradeable, interest-bearing ERC20 token with cDai as the backing token
 * and Dai as the underlying token. The V0 dDai exchange rate will grow at 90%
 ...
 * contract accounts via ERC-1271. The Dharma Dai V0 implementation is an
 * emergency fallback that pauses minting and pulling surplus cDai.
 */
contract DharmaDaiImplementationV0 is DharmaTokenV0 {
