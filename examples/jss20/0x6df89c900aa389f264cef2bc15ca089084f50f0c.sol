...
// File: browser/ETH_Balancer_MKRWETH_Zap_v1.sol
...
// interface
interface IBalancerGeneral_ETH_Balancer_MKRWETH_Zap_V1 {
...
contract ETH_Balancer_MKRWETH_Zap_V1 is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    bool private stopped = false;

    address public mkrWethPoolAddress = address(
        0x987D7Cc04652710b74Fff380403f5c02f82e290a
    );
    address public balancerGeneralZapAddress;

    constructor(address _balancerGeneralZapAddress) public {
        balancerGeneralZapAddress = _balancerGeneralZapAddress;
    }
    ...
    function LetsInvest(address _toWhomToIssue)
        public
        payable
        nonReentrant
        stopInEmergency
        returns (uint256 balancerPoolTokensRec)
    {
        balancerPoolTokensRec = IBalancerGeneral_ETH_Balancer_MKRWETH_Zap_V1(
            balancerGeneralZapAddress
        )
            .LetsInvest
            .value(msg.value)(_toWhomToIssue, mkrWethPoolAddress);
    }

    function changeBalancerGeneralZapAddress(address _balancerGeneralZapAddress)
        public
        onlyOwner
    {
        balancerGeneralZapAddress = _balancerGeneralZapAddress;
