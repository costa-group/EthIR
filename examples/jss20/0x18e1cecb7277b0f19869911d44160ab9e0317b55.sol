...
contract SafeMath {
	uint256 constant public MAX_UINT_256 = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;

	function safeAdd(uint256 x, uint256 y) pure internal returns (uint256 z) {
        if (x > MAX_UINT_256 - y) revert();
		return x + y;
	}
...
contract RipgaleToken is SafeMath {
    mapping(address => uint) public balances;
    mapping (address => mapping (address => uint256)) public allowance;
    
    string public name = "Ripgale";
    string public symbol = "RAL";
