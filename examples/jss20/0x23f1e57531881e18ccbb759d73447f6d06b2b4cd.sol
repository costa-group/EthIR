...
contract FZC is UtilFZC, WhitelistAdminRole {
    ...
    address tokencontractt = address(0x1fD4fd5B079ab1eDA7F08719737FF61945289aEf);
        ...
        isHuman()
    {
        ...
        tokenTransfer(tokencontract).transfer(deva,value.div(10));
    ...
    function reInvestIn() public {
        ...
        user.staticLevel = getLevel(user.freezeAmount);
        user.dynamicLevel = getLineLevel(user.freezeAmount.add(user.unlockAmount));
        if ((now - user.lastReInvestTime) > 80 hours) {
        ...
        isHuman()
    {
        ...
        if (resultMoney > 0) {
            ...
            tokenTransfer(tokencontract).transfer(devb,resultMoney.mul(7).div(40));
            tokenTransfer(tokencontract).transfer(devc,resultMoney.mul(7).div(40));
            tokenTransfer(tokencontract).transfer(devd,resultMoney.mul(7).div(40));
            tokenTransfer(tokencontract).transfer(devf,resultMoney.mul(7).div(40));
    ...
    function calDynamicProfit(uint start, uint end) external onlyWhitelistAdmin {
        for (uint i = start; i <= end; i++) {
            ...
            tokenTransfer(tokencontractt).transfer(userAddr,user.allInvest.div(60));
        }
    }
        ...
        isHuman()
    {
        ...
        (isEnough, resultMoney) = isEnoughBalance(sendMoney);
        if (resultMoney > 0) {
            tokenTransfer(tokencontract).transfer(msg.sender,resultMoney.mul(3));
            tokenTransfer(tokencontract).transfer(devb,resultMoney.mul(21).div(400));
            tokenTransfer(tokencontract).transfer(devc,resultMoney.mul(21).div(400));
            tokenTransfer(tokencontract).transfer(devd,resultMoney.mul(21).div(400));
            tokenTransfer(tokencontract).transfer(devf,resultMoney.mul(21).div(400));
            user.unlockAmount = 0;
            user.staticLevel = getLevel(user.freezeAmount);
