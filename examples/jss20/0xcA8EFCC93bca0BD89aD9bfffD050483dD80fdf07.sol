 ...
 * @title Requestable
 *
 * Enables contracts to easily accept signed requests and verify
 * their signer. Employs a nonce-pattern where requests must contain
 * a nonce (specific to their owner) that is one above the last nonce.
 * This can be verified by calling `validateRequest` at the start of a function
 *
 * Requests contain fee information (feeRecipient, feeToken & feeAmount)
 * which will be paid upon calling `completeRequest`. In order for this to
 * be possible, abstract functions `balanceOf` and `_payRequestFee` must
 * be implemented
 */
...
/*
 * Exposes operatorOf for use by child DepositContract
 */
