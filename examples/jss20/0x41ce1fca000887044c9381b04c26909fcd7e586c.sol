 ...
 *
 */
contract UsmtFeePool is Ownable , IERC20, MinterRole {
    ...
    constructor(address _usmtContract) public {
        usmtContract = USMT(_usmtContract);
    }
    ...
    function notifyTx(address _payer, uint256 _value) external onlySender(address(usmtContract)) returns (bool) {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (_to == address(this)) {
            uint256 currentPool = usmtContract.balanceOf(address(this));
            uint256 currentPZ = currentMined();
            uint256 result = currentPool.mul(_value).div(currentPZ);

            _transfer(msg.sender, _to, _value);

            usmtContract.feePoolTransfer(msg.sender, result);
...
contract USMT is Ownable, IERC20, MinterRole, ERC20Detailed {
    ...
    constructor() public ERC20Detailed("US Dollar in Manila Token", "USMT", 6) {
