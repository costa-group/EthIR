...
Features:
- MOOON holders get 33% from every buy event and 85%-33% from every sell event
- Anti-dump: selling fee gradually decreases from 85% at launch and 33% at the end of day 1.
- Referral bonus: 11.1% from buy-in amounts
- 100% open and secure
...
contract MOOON {
    ...
    //
    // prevents contracts from interacting with MOOON
    // 
    modifier isHuman() {
    ...
    uint8 constant internal entryFee_ = 33; // 15% to enter
    uint8 constant internal exitFeeHigh_ = 85; // 85% to exit (on the first day)
    uint8 constant internal exitFeeLow_ = 33; // 25% to exit (on the 30th day)
