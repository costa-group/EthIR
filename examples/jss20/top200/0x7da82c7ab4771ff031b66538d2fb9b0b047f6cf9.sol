...
contract MultiSigWallet {
    ...
    modifier signaturesFromOwners(bytes32 transactionHash, uint8[] v, bytes32[] rs) {
    	// GASTAP: Loop in a modifier. It depends on the lentgh of a parameter (uint8[] v)
    ...
    {
        isOwner[owner] = false;
	// GASTAP: Loop that depends on the lentgh of a field of type (address [])
    ...
    {
	// GASTAP: Loop that depends on a parameter (address[] _owners)    
    ...
    {
        uint count = 0;
	// GASTAP: Loop that depends on a field (address[] owners)    
	...
	// GASTAP: Function with two consecutive loops that depends on a field bytes32[] transactionList and a parameter bytes32[] _transactionList
