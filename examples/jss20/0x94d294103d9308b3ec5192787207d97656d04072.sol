...
// File: original_contracts/lib/maker/IMakerDAO.sol
...
// File: original_contracts/lib/maker/Maker.sol
...
contract Maker is IExchange {
        ...
        returns (uint256)
    {
        ...
        if (address(fromToken) == SAI){
            require(address(toToken) == DAI, "Destination token should be DAI");
            IMakerDAO(exchange).swapSaiToDai(fromAmount);
        }
        else if (address(fromToken) == DAI) {
            require(address(toToken) == SAI, "Destination token should be SAI");
            IMakerDAO(exchange).swapDaiToSai(fromAmount);
        }
        else {
