...
contract StableAMM is ERC20, ERC20Detailed, ReentrancyGuard {
    ...
    address public constant unirouter = address(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f );
    ...
    function deposit(address token, uint amount) external nonReentrant {
        ...
        uint value = Oracle(link).getPriceUSD(token).mul(amount).div(ERC20Detailed(token).decimals());
    ...
    function withdraw(address token, uint amount) external nonReentrant {
        ...
        uint valueA = Oracle(link).getPriceUSD(token).mul(amountA).div(ERC20Detailed(token).decimals());
        if (valueA > amountB) {
            valueA = amountB;
        }
