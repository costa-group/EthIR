...
/*
          ...
          if(liquidityProviderContractAddress == fundsReturnToAddress){
            ...
            if(equal(exchanges[0], "SKIP")){
              string[]   memory newTokens = new string[](tokens.length-1);
               ...
               //doesnt work so lets go with literal due to memory types restruct etc
               //newTokens = remove(0, tokens);
              //newExchanges = remove(0, exchanges);
    ...
    /*
       function remove2(uint index, string[] storage array) internal returns(string[]) {
        if (index >= array.length) return;
...
/*
  ...
  function compare(string _a, string _b) returns (int) {
    ...
    ERC20 theToken = ERC20(_reserve);
      ...
      //  string memory amountString = uint2str(amountAndFee);
       // string memory thisContractBalanceNow1 = uint2str(thisContractBalanceNow);
        //require(thisContractBalanceNow < amountAndFee, "The transaction did not return a profit + fee of "+  amountString+ ". Amount returned was "+ thisContractBalanceNow1 );
