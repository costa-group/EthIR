...
contract SmartMatrixForsage {
    ...
    struct User {
        ...
        mapping(uint8 => bool) noRFilledLevels;
    ...
    event Filled(address indexed newUser, address indexed caller, uint8 matrix, uint8 level);
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) {
            ...
            require(users[msg.sender].activeX3Levels[level-1], "You need to activate previous level to buy this one");
            ...
            require(users[msg.sender].activeX6Levels[level-1], "You need to activate previous level to buy this one");
            ...
            emit Upgrade(msg.sender, xNoRMatrix[level].currentUser, 3, level);

        }
    }    
    
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == 0.06 ether, "registration cost 0.06");
    ...
    function updateNoReinvestReferrer(address userAddress, uint8 level) private {
        ...
        emit NewUserPlace(userAddress, xNoRMatrix[level].currentUser, 3, level, uint8(xNoRMatrix[level].referrals.length));
        sendETHDividends(xNoRMatrix[level].currentUser, userAddress, 3, level);
        if (xNoRMatrix[level].referrals.length < 3) {
            return;
        }

        users[xNoRMatrix[level].currentUser].noRFilledLevels[level] = true;
        
        xNoRMatrix[level].referrals = new address[](0);
        xNoRMatrix[level].currentUser = dequeueToNoR(level);
        emit Filled(xNoRMatrix[level].currentUser, userAddress, 3, level);
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if(referrerAddress != owner && referrerAddress != address(0)) {
            users[users[referrerAddress].x6Matrix[level].currentReferrer].x6Matrix[level].closedPart = referrerAddress; // reduced a lot of code
        }
        // if (x6.length == 2) { // ??? What is this condition for? What is closedPart?
        //     if (x6[0] == referrerAddress ||
        //         x6[1] == referrerAddress) {
        //         users[users[referrerAddress].x6Matrix[level].currentReferrer].x6Matrix[level].closedPart = referrerAddress;
        //     }
        // } else if (x6.length == 1) { // ??? Why is this here if to get here x6.length == 2?
        //         if (x6[0] == referrerAddress) {
        //             users[users[referrerAddress].x6Matrix[level].currentReferrer].x6Matrix[level].closedPart = referrerAddress;
        //         }
        // }
    ...
    function usersFilledNoRLevels(address userAddress, uint8 level) public view returns(bool) {
        return users[userAddress].noRFilledLevels[level];
    }
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (!address(uint160(receiver)).send(levelPrice[level])) { // ??? What is this check?
