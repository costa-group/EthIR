...
contract xyl is MintableToken, HasNoEther 
{
    // Using libraries 
    using SafeMath for uint;

    //////////////////////////////////////////////////
    /// State Variables for the xyl token contract
    //////////////////////////////////////////////////
    ...
    */
    ...
    // Human token state
    //////////////////////
    string public constant name = "xyl.finance";
    string public constant symbol = "xylk";
    ...
    //////////////////////////////////////////////////
    /// Begin xyl token functionality
    //////////////////////////////////////////////////

    /// @dev xyl token constructor
    function xyl() public
    {
        // Define owner
        owner = msg.sender;
        // Define initial owner supply. (ether here is used only to get the decimals right)
        uint _initOwnerSupply = 10000 ether;
    ...
    function stakexyl(uint _stakeAmount) external
    {
    ...
    function stakexylSplit(uint _stakeAmount, address _stakeSplitAddress) external
    {
        // Require that a xyl split actually be split with an address
    ...
    function claimxyl() external returns (bool success)
    {
        /// Sanity checks: 
        ...
        // Subtract stake balance from totalxylStaked
        totalxylStaked -= stakeBalances[msg.sender].initialStakeBalance;
    ...
    function stakeTokens(uint256 _value) private returns (bool success)
    {
        ...
        totalxylStaked += _value;

        /// Determine percentage of global stake
        stakeBalances[msg.sender].initialStakePercentage = calculateFraction(_value, totalxylStaked, decimals);
    ...
    function calculateStakeGains(uint _timePassedSinceStake) view private returns (uint mintTotal)
    {
        ...
        uint _finalMintRate;            // store our calculated final mint rate (in xyl-per-second)
        ...
        if (_timePassedSinceStake > _secondsPerDay) {
            ...
            // First, calculate our final stake percentage based upon the total amount of xyl staked
            _finalStakePercentage = calculateFraction(stakeBalances[msg.sender].initialStakeBalance, totalxylStaked, decimals);
            ...
            // Finally, calculate our final mint rate (in xyl-per-second)
