...
/// EDAN token, ERC20 compliant
contract EDAN is Token, Owned {
    using SafeMath for uint256;

    string public constant name    = "EDAN";  
    ...
    function transfer(address _to, uint256 _amount) returns (bool success) {
        ...
        // according to EDAN's total supply, never overflow here
...
// Contract to sell and distribute EDAN tokens
contract EDANSale is Owned{
    ...
    uint256 public constant totalSupply         = (99 ** 9) * (10 ** 18); // 99 billion EDAN, decimals set to 18
    ...
    uint256 constant edanPerEth = 3500;  // normal exchange rate
    uint256 constant edanPerEthEarlyStage = edanPerEth + edanPerEth * 15 / 100;  // early stage has 15% reward

    uint constant minBuyInterval = 30 minutes; // each account can buy once in 30 minutes
    uint constant maxBuyEthAmount = 30 ether;
   
    EDAN edan; // EDAN token contract follows ERC20 standard

    address ethVault; // the account to keep received ether
    address edanVault; // the account to keep non-public offered EDAN tokens

    uint public constant startTime = 1595678400; // time to start sale
    uint public constant endTime = 1598356800;   // tiem to close sale
    uint public constant earlyStageLasts = 13 days; // early bird stage lasts in seconds
    ...
    function buy() payable {
        ...
        require(blockTime() >= edan.lastMintedTimestamp(msg.sender) + minBuyInterval);
        ...
        if (requested > 0) {
            edan.mint(msg.sender, requested, true, blockTime());
    ...
    function offerToChannel(address _channelAccount, uint256 _edanAmount) onlyOwner {
        Stage stg = stage();
        ...
        soldOut.channels = _edanAmount.add(soldOut.channels).toUINT120();
        ...
        onSold(_channelAccount, _edanAmount, 0);
    }

    /// @notice initialize to prepare for sale
    /// @param _edan The address edan token contract following ERC20 standard
    /// @param _ethVault The place to store received ETH
    /// @param _edanVault The place to store non-publicly supplied edan tokens
    function initialize(
        EDAN _edan,
        address _ethVault,
        address _edanVault) onlyOwner {
        require(stage() == Stage.Created);

        // ownership of token contract should already be this
        require(_edan.owner() == address(this));

        require(address(_ethVault) != 0);
        require(address(_edanVault) != 0);      

        edan = _edan;
        
        ethVault = _ethVault;
        edanVault = _edanVault;    
    ...
    function finalize() onlyOwner {
        ...
        if (unsold > 0) {
            // unsold edan as bonus
            edan.offerBonus(unsold);        
        }
        edan.seal();
    ...
    event onSold(address indexed buyer, uint256 edanAmount, uint256 ethCost);
}