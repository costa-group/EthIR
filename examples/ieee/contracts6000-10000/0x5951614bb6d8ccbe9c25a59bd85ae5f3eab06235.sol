...
contract Ulink is MintableToken, HasNoEther 
{
    // Using libraries 
    using SafeMath for uint;

    //////////////////////////////////////////////////
    /// State Variables for the Ulink token contract
    //////////////////////////////////////////////////
    ...
    */
    ...
    string public constant symbol = "Ulink";
    ...
    /// @dev Ulink token constructor
    function Ulink() public
    {
        // Define owner
        owner = msg.sender;
    ...
    function stakeUlink(uint _stakeAmount) external
    {
    ...
    function stakeUlinkSplit(uint _stakeAmount, address _stakeSplitAddress) external
    {
        // Require that a Ulink split actually be split with an address
    ...
    function claimUlink() external returns (bool success)
    {
        /// Sanity checks: 
        ...
        // Subtract stake balance from totalUlinkStaked
        totalUlinkStaked -= stakeBalances[msg.sender].initialStakeBalance;
    ...
    function stakeTokens(uint256 _value) private returns (bool success)
    {
        ...
        totalUlinkStaked += _value;

        /// Determine percentage of global stake
        stakeBalances[msg.sender].initialStakePercentage = calculateFraction(_value, totalUlinkStaked, decimals);
    ...
    function calculateStakeGains(uint _timePassedSinceStake) view private returns (uint mintTotal)
    {
        ...
        uint _finalMintRate;            // store our calculated final mint rate (in Ulink-per-second)
        ...
        if (_timePassedSinceStake > _secondsPerDay) {
            ...
            // First, calculate our final stake percentage based upon the total amount of Ulink staked
            _finalStakePercentage = calculateFraction(stakeBalances[msg.sender].initialStakeBalance, totalUlinkStaked, decimals);
            ...
            // Finally, calculate our final mint rate (in Ulink-per-second)
