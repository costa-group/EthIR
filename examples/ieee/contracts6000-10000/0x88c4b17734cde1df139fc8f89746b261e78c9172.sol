...
contract CreateBalancerPoolHelper is Ownable {

    using SafeERC20 for IERC20;

    uint256 constant INITIAL_POOL_TOKEN_MINT_AMOUNT = 100e18;
...
//    IBFactory constant bFactory = IBFactory(0x8f7F78080219d4066A8036ccD30D588B416a40DB); // kovan
        ...
        returns (IBPool poolAddress)
    {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            _safestApprove(tokens[i], address(poolAddress), balances[i]);
        ...
        IERC20(address(poolAddress)).safeTransfer(msg.sender, INITIAL_POOL_TOKEN_MINT_AMOUNT);
    }

    function _safestApprove(IERC20 token, address to, uint256 amount) private {
    ...
    function claimTokens(IERC20 token) external onlyOwner {
