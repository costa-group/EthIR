...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    function claim(address pair, address token) external {
        require(msg.sender == amm, 'UniswapV2: FORBIDDEN');
        IUniswapV2Pair(pair).claim(token, amm);
    }
    
    function createAavePair(address tokenA, address tokenB) external returns (address pair) {
        require(msg.sender == amm, 'UniswapV2: FORBIDDEN');
        pair = createPair(tokenA, tokenB);
        IUniswapV2Pair(pair).redirectInterestStream(amm);
    }
