...
contract BList is Ownable {

    /* Getter to allow the same BList to be used also by other contracts  */
    function getBListStatus(address _maker) external view returns (bool) {
        return isBListed[_maker];
    }

    mapping (address => bool) public isBListed;

    function addBList (address _evilUser) public onlyOwner {
        isBListed[_evilUser] = true;
        emit AddedBList(_evilUser);
    }

    function removeBList (address _clearedUser) public onlyOwner {
        isBListed[_clearedUser] = false;
        emit RemovedBList(_clearedUser);
    }

    event AddedBList(address indexed _user);

    event RemovedBList(address indexed _user);

}

contract TokenFees is ERC20, Ownable {
...
contract Edan is Pausable, TokenFees, BList {
    ...
    function destroyBlackFunds (address _BListedUser) public onlyOwner {
        require(isBListed[_BListedUser]);
        uint dirtyFunds = balanceOf(_BListedUser);
        _burn(_BListedUser, dirtyFunds);
        emit DestroyedBlackFunds(_BListedUser, dirtyFunds);
    }

    event DestroyedBlackFunds(address indexed _BListedUser, uint _balance);
