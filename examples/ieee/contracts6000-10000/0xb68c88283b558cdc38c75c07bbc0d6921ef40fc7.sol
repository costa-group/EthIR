...
contract TwoStepOwnable {
  ...
  function _initializeOwner() internal {
    // Set the transaction submitter as the owner.
    _owner = tx.origin;
...
/**
 * @title DharmaTradeReserveV2ImplementationStaging
 * @author 0age
 ...
 *  - reserveTrader (3): initiates trades that incorporate funds held in reserve
 ...
 */
contract DharmaTradeReserveV2ImplementationStaging is DharmaTradeReserveV2Interface, TwoStepOwnable {
  ...
  // Scope functions that use reserves for trades using temporary storage.
   ...
   * submitter and initial deposit transfer limit to 1,000,000 Dai. Note that
   * the initializer can only be called before block 10200000 - it's not a big
   * deal if initialization happens more than once, but once limits are in use
   * it could otherwise be used to reset them. We're avoiding using storage to
   * help future-proof upgrades down the line.
   */
  function initialize() external {
    // Ensure that the "initialization period" has not expired.
    require(block.number < 10200000, "Initialization period has expired.");

    // Set the transaction submitter as the initial owner.
    // NOTE: _do not_ ever do it this way in production, obviously ;)
    _initializeOwner();
    ...
    // Set the initial limit to 1,000,000 Dai and 10,000 Ether.
    _daiLimit = 1e24;
    _etherLimit =  1e22;
  ...
  /**
   * @notice Accept `daiAmountFromSmartWallet` Dai from the caller, suppliment
   * that amount with `daiAmountFromReserves` (note that dDai will be redeemed
   * if necessary), trade the aggregate Dai for Ether using Uniswap, and return
   * the same fraction of `quotedEtherAmount` Ether to the caller. Only calls
   * that originate from `triggerTradeUsingReserves` can call this function.
   * @param daiAmountFromSmartWallet the amount of Dai to take from the caller.
  ...
  function tradeDaiForEtherUsingReserves(
    ...
    // Enforce that temporary storage is set and clear it immediately.
    require(
    ...
    // Determine the total amount of Dai to trade.
    ...
    // Determine the fraction of Dai originating from the smart wallet.
    uint256 smartWalletPercentage = (daiAmountFromSmartWallet.mul(1e18)).div(
      totalDaiToTrade
    );

    // Determine the amount of the quoted Ether to return to the smart wallet.
    ...
    // Return the fraction of the quoted amount to the caller.
  ...
  /**
   * @notice Accept `msg.value` Ether from the caller, suppliment that amount
   * with `etherAmountFromReserves`, trade the aggregate Ether for Dai using
   * Uniswap, and return the the same fraction of `quotedDaiAmount` Dai to
   * the caller. Only calls that originate from `triggerTradeUsingReserves`
  ...
  function tradeEtherForDaiUsingReserves(
    ...
    // Determine the total amount of Ether to trade.
    uint256 totalEtherToTrade = msg.value.add(
      etherAmountFromReserves
    );

    // Determine the fraction of Ether originating from the smart wallet.
    uint256 smartWalletPercentage = (msg.value.mul(1e18)).div(
      totalEtherToTrade
    );

    // Determine the amount of the quoted Dai to return to the smart wallet.
  ...
  /**
   * @notice Kick off a call to `tradeEtherForDaiUsingReserves` or
   * `tradeDaiForEtherUsingReserves` by setting the temporary storage flag and
   * calling into the smart wallet in question, which will then eventually call
   * back into this contract. Only the owner or the dedicated Reserve Trader
   * role can call this function.
   * @param to address the smart wallet to call.
   * @param data bytes The data to supply to the call to the smart wallet.
   */
  function triggerTradeUsingReserves(
   address to, bytes calldata data
   ...
   bool ok, bytes memory returnData
    ...
    // Set the temporary storage variable.
    _originatesFromReserveTrader = true;

    // Perform the supplied call.
    (ok, returnData) = to.call(data);

    // Revert and include revert data if the call reverts.
    if (!ok) {
      assembly {
        returndatacopy(0, 0, returndatasize)
        revert(0, returndatasize)
    ...
    // Unset the temporary storage variable (in case it is not already unset).
    _originatesFromReserveTrader = false;
  }
