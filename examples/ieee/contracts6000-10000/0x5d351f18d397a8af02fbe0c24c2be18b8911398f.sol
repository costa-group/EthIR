...
contract ERC20 is IERC20 {
    ...
    */
     ...
     * @param from address The address which you want to send Nordic Energy tokens from
...
/**
 * Nordic Energy token upgrader interface.
 *
 * Nordic Energy token upgrader transfers previous version Nordic Energy tokens to a newer version.
 * Token upgrader itself can be the Nordic Energy token contract, or just a middle man contract doing the heavy lifting.
 */
...
contract UpgradeableToken is MintableAndPausableToken {
    ...
    // The next contract where the Nordic Energy tokens will be migrated.
    ...
    * - Waiting: Nordic Energy token allows upgrade, but we don't have a new Nordic Energy token version
    * - ReadyToUpgrade: The Nordic Energy token version is set, but not a single token has been upgraded yet
    * - Upgrading: Nordic Energy token upgrader is set and the balance holders can upgrade their tokens
    */
    ...
    // New Nordic Energy token version available.
    ...
    // Allow the Nordic Energy token holder to upgrade some of their tokens to a new contract.
    function upgrade(uint _value) external {
        ...
        // Nordic Energy Token Upgrader reissues the tokens
...
contract NordicEnergyToken is UpgradeableToken, ERC20Burnable {
    ...
    /**
    * Owner can update Nordic Energy token information here
    */
    ...
    function signUpForRefund( uint256 _value ) public {
        require( hodlPremium[msg.sender].hodlTokens != 0 || hodlPremium[msg.sender].buybackTokens != 0, "You must be ICO/STO user to sign up" ); //the user was registered in ICO/STO
    ...
    function refund( uint256 _value ) public {
        ...
        _burn( msg.sender, _value );
        require( stablecoin.transferFrom( stablecoinPayer, msg.sender, _value.div(20) ), "transfer failed" ); //we pay 1/20 = 0.05 DAI for 1 NET
    }
    ...
    /**
    * Owner can burn Nordic Energy token here
    */
    ...
    function transfer(address _to, uint256 _value) public whenNotPaused returns (bool) {
        ...
        if (hodlPremiumMinted < hodlPremiumCap && hodlPremium[msg.sender].hodlTokens > 0) {
            ...
            //subtract the Nordic Energy tokens token into account here to avoid the above calculations in the future, e.g. in case I withdraw everything in 0 days (bonus 0), and then refund, I shall not be eligible for any bonuses
