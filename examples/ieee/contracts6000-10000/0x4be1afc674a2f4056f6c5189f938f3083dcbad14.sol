...
contract yDelegatedVault is ERC20, ERC20Detailed {
    ...
    function availableToBorrowReserve() external view returns (uint) {
        ...
        if (_maxSafeETH > _totalBorrowsETH) {
            ...
            return _available.mul(ERC20Detailed(_reserve).decimals()).div(getReservePriceETH(_reserve));
        } else {
            return 0;
        }
    }
    ...
    function earn() external {
        ...
        if (_maxSafeETH > _totalBorrowsETH) {
            uint _available = _availableBorrowsETH.mul(_maxSafeETH.sub(_totalBorrowsETH)).div(_availableBorrowsETH);
            _available = _available.mul(ERC20Detailed(_reserve).decimals()).div(getReservePriceETH(_reserve));
