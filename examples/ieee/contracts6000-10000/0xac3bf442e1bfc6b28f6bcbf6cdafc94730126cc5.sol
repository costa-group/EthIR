...
/**
 * @dev {Aeron} upgradable token contract
 */

contract Aeron is Initializable, ContextUpgradeSafe, ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    function initialize(string memory name, string memory symbol) public initializer {
        __AeronToken_init_unchained(name, symbol);
    }

    function __AeronToken_init_unchained(string memory name, string memory symbol) internal initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __ERC20_init_unchained(name, symbol);
        _mint(_msgSender(), 200000000000000000000000000);
    }
   ...
   function mint(address to, uint256 amount) public {
        require((_msgSender() == _staking_contract || (_msgSender() == owner() && to == owner() && _frozen[to] >= amount)),
            "Caller is not a minter");
        _mint(to, amount);
    }

    function burn(address from, uint256 amount) public {
        require((_msgSender() == _staking_contract || (_msgSender() == owner() && from == owner())),
            "Caller is not a burner");
        _burn(from, amount);
    }

    function transferERC20(IERC20 _token, address to, uint256 amount) public onlyOwner returns (bool) {
        require(_token.transfer(to, amount), "Token transfer failed");
    }

    function freeze(uint256 _value) public onlyOwner {
    ...
    function unfreeze(uint256 _value) public onlyOwner {
