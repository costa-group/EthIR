...
library SetLibrary
{
    ...
    function remove(Set storage self, uint256 value) public returns (bool removed)
    {
        ...
        {
            self.values[self.valuesMapping[value].index] = self.values[self.values.length-1]; // SAFEVM Array Access with map content
        }
...
contract SetUsageExample
{
    ...
    {
	require (index < numberCollection.values.length); // SAFEVM Added
