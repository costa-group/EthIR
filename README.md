...
```
If we execute the command `./oyente_ethir -s ../examples/blockking.evm.disasm -disasm -eop` instead of the above one, the RBR that EthIR produces has nops annotations, with the EVM bytecode interleaved in the text. Below is a sketch of the RBR with nops annotations generated for *blockking.evm.disasm*:
```
block0(g(11), g(10), g(9), g(8), g(7), g(6), g(5), g(4), g(3), g(2), g(1), g(0), l(8), l(7), l(6), l(5), l(4), l(3), l(2), l(1), l(0), calldatasize, calldataload, gas, caller, callvalue, number, gasprice, balance)=>
	s(0) = 96
	nop(PUSH1)
...
EthIR also allows us to store the CFG of the file analysed. In that case, we have to execute the command `./oyente_ethir -s ../examples/blockking.evm.disasm -disasm -cfg`. EthIR stores the cfg in a file with *.cfg* extension in the directory */tmp/costabs/*.
