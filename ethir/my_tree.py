...
class CollapsedTree:
    ...
    def add_node_to_graph(self, node: BasicBlock):
        ...
        if isinstance(jump, str) or jump > 0:
            if node.get_block_type() == "conditional":
                self.tree_structure += (
                    f'n_{node.get_start_address()} -> n_{jump} [label="t"];\n'
                )
            else:
                self.tree_structure += (
                    f'n_{node.get_start_address()} -> n_{jump} [label=""];\n'
                )
        ...
        if falls_to is not None:
            if node.get_block_type() == "conditional":
                self.tree_structure += (
                    f'n_{node.get_start_address()} -> n_{falls_to} [label="f"];\n'
                )
            else:
                self.tree_structure += (
                    f'n_{node.get_start_address()} -> n_{falls_to} [label=""];\n'
                )
    ...
    def generate_dot(self):
        ...
        with open(f"{name}/Collapsed{self.file_name}.dot", "w") as f:
            f.write("digraph id3{ \n")
