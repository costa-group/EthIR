...
class BlockAnalysisInfo:
    ...
    def process_block(self):
        ...
        for i, instr in enumerate(instructions):
            ...
            current_state = current_state.process_instruction(
                instr, str(idblock) + ":" + str(i)
            )
            if debug_info:
                print(
                    f"      -- {self.block_info.get_start_address()}[{i}] {instr} -- {current_state}"
                )
    ...
    def __repr__(self):
        ...
        for state in self.state_per_instr:
            print(
                str(self.block_info.get_start_address())
                + "."
                + str(i)
                + ": "
                + str(self.state_per_instr[i])
            )
            i = i + 1
...
class Analysis:
    ...
    def process_jumps(self, block_id, input_state):
        ...
        if (jump_target != 0 and jump_target != -1) and self.blocks_info.get(
            jump_target
        ) == None:
            ...
            self.blocks_info[jump_target] = BlockAnalysisInfo(
                self.vertices[jump_target], input_state
            )

        elif (jump_target != 0 and jump_target != -1) and self.blocks_info.get(
            jump_target
        ).revisit_block(input_state, jump_target):
        ...
        elif jump_target != None and self.blocks_info.get(jump_target).revisit_block(
            input_state, jump_target
        ):
            self.pending.append(jump_target)
...
def perform_memory_analysis(
    ...
    print(
        "Memory read accesses Contract"
        + cname
        + ": "
        + str(len(accesses.readset.keys()))
    )
    print(
        "Memory write accesses Contract"
        + cname
        + ": "
        + str(len(accesses.writeset.keys()))
    )
