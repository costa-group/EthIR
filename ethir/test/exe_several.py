...
def check_timefile_exist():
    files = os.listdir(".")
    if "time_subset" not in files:
        f = open("time_subset.csv","a")
        fp = csv.writer(f, delimiter=',')
...
if __name__ == "__main__":
    ...
    l = os.listdir(".")
    if "time_subset.csv" in l:
        os.remove("time_subset.csv")
    if "result_subset.csv" in l:
        os.remove("result_subset.csv")
    if "report_subset.txt" in l:
        os.remove("report_subset.txt")

    check_csvfile_exist()
    check_timefile_exist()
    
    path = "/home/pablo/Escritorio/examples/subset/"
    ethir_path = "/home/pablo/EthIR/ethir/"	
    sol_files = os.listdir("/home/pablo/Escritorio/examples/subset/")
    files = len(sol_files)
